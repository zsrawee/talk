...
const postSchema = z.object({
  title: z
    ...
    .max(20, "العنوان يجب أن يكون 20 حرف على الأكثر"),
  content: z
    .string()
...
export async function GET(req: Request) {
  ...
  const limit = Math.min(
    50,
    Math.max(1, parseInt(url.searchParams.get("limit") || "9"))
  );
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Support both multi-language and single-language creation
    let translations: { language: Lang; title: string; content: string }[];

    if (body.translations && Array.isArray(body.translations) && body.translations.length > 0) {
      translations = body.translations;
    } else {
      // Legacy single-language mode
      const { title, content, language } = postSchema.parse(body);
      translations = [{ language: language as Lang, title, content }];
    }
